...
void server::cl_invite(Clients &client)
{
	if (!client.registred)
	{
		client.out_buf += ":server 451 " + client.nickname + " INVITE :You have not registered\r\n";
		return;
	}
	...
	if (it == this->channels.end())
	{
		...
		it = this->channels.find(chanName);
	}
	...
	client.out_buf += ":server 341 " + client.nickname + " " + targetNick + " " + chanName + "\r\n";
